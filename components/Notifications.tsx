...
const Notifications = () => {
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="w-[460px] border-none bg-white/15 backdrop-blur-2xl text-white shadow-lg">
        <LiveblocksUIConfig 
          overrides={{
          ...
          <InboxNotificationList>
            ...
            {unreadNotifications.length > 0 && unreadNotifications.map((notification) => (
              <InboxNotification 
                ...
                className="bg-[url('/assets/doc.png')] bg-cover text-white"
                href={`dashboard/documents/${notification.roomId}`}
                showActions={false}
                kinds={{
                  thread: (props) => (
